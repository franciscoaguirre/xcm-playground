...
pub fn sibling_account_account_id(para: u32, who: sp_runtime::AccountId32) -> parachain::AccountId {
	let location = (Parent, Parachain(para), AccountId32 { network: Some(NetworkId::Kusama), id: who.into() });
	parachain::LocationToAccountId::convert_location(&location.into()).unwrap()
}
...
pub fn para_ext(para_id: u32) -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Runtime> {
		balances: vec![
			...
			(sibling_account_account_id(other_para_id, ALICE), INITIAL_BALANCE),
		]
	}
